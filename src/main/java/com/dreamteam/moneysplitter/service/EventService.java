...
public class EventService {
    ...
    public Boolean addUserToEvent(Long eventId, Long userId, String principal) {
        ...
        Event event = eventRepo.findById(eventId).orElseThrow();
        if (userEventRepo.findAllByEvent(event).stream().anyMatch(e -> e.getUser().equals(inviter))) {
            addUserToEvent(invitedUser, event);
            return true;
        }
        return false;
    }
    ...
    public boolean deleteUserFromEvent(Long eventId, Long userId, String principal) {
        ...
        Event event = eventRepo.findById(eventId).orElseThrow();
        if (event.getOwner().equals(owner) && userEventRepo.findAllByEvent(event).stream().anyMatch(e -> e.getUser().equals(deletedUser))) {
    ...
    private void addUserToEvent(User user, Event event) {
    ...
    public boolean addPurchaseToEvent(Long eventId, PurchaseDTO purchaseDTO, String principal) {
        User user = userRepo.findByEmail(principal);
        Event event = eventRepo.findById(eventId).orElseThrow();
        if(userEventRepo.findAllByEvent(event).stream().anyMatch(e -> e.getUser().equals(user))){
    ...
    public boolean deletePurchaseFromEvent(Long eventId, Long purchaseId, String principal) {
        ...
        if(purchase.getEvent() != null && purchase.getEvent().equals(event) && purchase.getUser().equals(user)){
